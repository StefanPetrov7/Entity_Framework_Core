...
INSERT INTO Villains (Name, EvilnessFactorId) VALUES (@Name, 4)
...
CREATE OR ALTER PROCEDURE usp_DeleteVillain(@Id INT)
AS
BEGIN TRANSACTION 

IF NOT EXISTS (SELECT * FROM Villains WHERE Id = @Id)
BEGIN
    ROLLBACK;
    THROW 50001, 'No such villain was found.',1
END

DELETE FROM MinionsVillains WHERE VillainId = @Id

DELETE FROM Villains WHERE Id = @Id

COMMIT

SELECT COUNT(*) FROM MinionsVillains WHERE VillainId = @Id

SELECT Name FROM Villains WHERE Id = @Id

EXEC dbo.usp_DeleteVillain @Id
...
SELECT Name FROM Minions WHERE Id = @Id
